...
func TestSaveToFile(t *testing.T) {
	ini, err := Load(strings.NewReader(src))
	if err != nil {
		t.Fatal(err)
	}

	err = ini.Set("new section", "new key", "new value")
	if err != nil {
		t.Fatal(err)
	}

	err = ini.SaveToFile(save_file)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestRemove(t *testing.T) {
	...
	err = ini.Remove("new section", "new key")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = ini.Remove("new section", "new key")
	if err == nil {
		t.Fatal("No error reported for non-exist value")
	}
}
