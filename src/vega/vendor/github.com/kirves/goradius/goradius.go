...
// Authenticate authenticates a user against the Radius server and returns true whether the user provided the correct password
// Also returns the privilege level, error is privilege information is not available
func (a *AuthenticatorT) AuthenticateWithPrivilege(username, password string) (int, bool, error) {
	url := fmt.Sprintf("%s:%s", a.server, a.port)
	conn, err := net.DialTimeout("udp", url, 0)
	if err != nil {
		return -1, false, err
	}
	defer conn.Close()
	...
	msg := a.createRequest(auth, []byte(username), encpass, []byte(""))
	...
	for i := 0; i < RETRIES; i++ {
		...
		select {
		case <-ch:
			accepted, err := a.parseResponse(resp, auth)
			if err != nil || !accepted {
				return -1, false, err
			}
			privilege, err := a.parsePrivilege(resp)
			return privilege, true, err
		case err := <-eCh:
			return -1, false, err
		case <-time.After(10 * time.Second):
			return -1, false, errors.New("Timed out while waiting for an answer")
		}
	}
	return -1, false, nil
}

func (a *AuthenticatorT) parsePrivilege(resp []byte) (int, error) {
	startingIndex := 20
	invalidError := errors.New("Forged or corrupted response")

	for idIndex := startingIndex; idIndex < len(resp)-1; {
		id, length := resp[idIndex], resp[idIndex+1]
		...
		length = length - 2
		endIndex := idIndex + 1 + int(length)

		if len(resp)-1 < endIndex {
		...
		if id == 180 {
			...
			privilege := binary.BigEndian.Uint32(resp[idIndex+2 : endIndex+1])
			return int(privilege), nil
		}
		idIndex = idIndex + int(length) + 2
	}

	return -1, errors.New("Privilege attribute not available")
}
